    ...
    def export_excel(self, fiches_data: List[Dict[str, Any]], output_path: str | Path) -> str:
        ...
        for f in fiches_data:
            rows.append({
                ...
                "Information Trouvée (Français)": f.get("information_found", ""),
                "Synthèse Officielle THE (Anglais)": f.get("english_summary_for_the", ""),
                "Année": f.get("year", ""),
                "Source Exacte": f.get("source_exact", ""),
                "Citation Justificative (Verbatim)": f.get("justifying_quote", ""),
                "Traduction Anglaise Citation": f.get("quote_english_translation", ""),
    ...
    def export_word(self, pilot_data: Dict[str, Any], output_path: str | Path) -> str:
        ...
        for f in pilot_data.get("fiches", []):
            ...
            fields = [
                ...
                ("Information trouvée (Français)", f.get("information_found")),
                ("🇬🇧 Official THE Portal Summary", f.get("english_summary_for_the")),
                ("Année concernée", str(f.get("year", "N/A"))),
                ("Source exacte", f.get("source_exact")),
                ("Extrait justificatif (verbatim)", f.get("justifying_quote")),
                ("🇬🇧 Quote English Translation", f.get("quote_english_translation")),
